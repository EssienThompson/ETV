...
const SPRINT = 1.75
const SPRINT_ACC = 1.65
const SWORD_SPEED = 1.0#1.35
...
@onready var hitbo: hitbox = $man/rig/Skeleton3D/BoneAttachment3D/swordCont/longerSword/hitbox
...
var swing1MoveDist := 0.45
var swing2MoveDist := 0.45
var swing3MoveDist := 0.45 
var swing4MoveDist := 0.45
...
func _physics_process(delta):
	...
	if not is_on_floor():
		...
		if velocity.y < 0:
			appGrav *= 1.5
		velocity.y -= appGrav*delta
		velocity.y = max(velocity.y, -MAX_FALL_SPEED)
		#velocity.y -= 1.5 * jumpGrav * delta
	else:
		velocity.y -= 0.1#move_toward(velocity.y, gravity, DECELERATION*delta)#gravity * delta
		isJumping = false
	...
	if hitStunned:
		...
		if isfallAttacking:
			...
			if swingSprt:
				...
				velocity = velocity.move_toward(attackVec/1.5, ACCELERATION*delta)
			...
			if swing4Move:
				...
				velocity = velocity.move_toward(attackVec, ACCELERATION*delta)
				
			elif (atkTimer >= 0.8/SWORD_SPEED && atkTimer <= 1.1/SWORD_SPEED) or isBlocking:
				inBetweenAtk = true
			...
			elif (atkTimer >= 0.75/SWORD_SPEED && atkTimer <= 1.05/SWORD_SPEED) or isBlocking:
				inBetweenAtk = true
				inBetweenThreshold = 0.95/SWORD_SPEED
				isAttacking = false
			...
			elif (atkTimer >= 0.8/SWORD_SPEED && atkTimer <= 1.15/SWORD_SPEED) or isBlocking:
				inBetweenAtk = true
				inBetweenThreshold = 0.95/SWORD_SPEED
				isAttacking = false
		...
		if atkDodged:
			...
			velocity = velocity.move_toward(dodgeVec, ACCELERATION*delta)
			#velocity.x = dodgeVec.x
			#velocity.z = dodgeVec.z
		else:
			velocity = velocity.move_toward(dodgeVec/4, ACCELERATION*delta)
		######################################################################################
	elif hitBlocked:
		blockHitTimer += delta
		...
		velocity = velocity.move_toward(direction * SPEED * SPRINT, ACCELERATION*SPRINT_ACC*delta)
	elif isJumping and isSprint and !isAttacking && !performDodge && !mapOpen:
		velocity.x = (direction.x * SPEED * SPRINT)
		velocity.z = (direction.z * SPEED * SPRINT)
		#velocity = velocity.move_toward(direction * SPEED * SPRINT, ACCELERATION*delta)
		...
		#velocity = velocity.move_toward((direction/2) * SPEED * SPRINT, ACCELERATION*delta)
...
func _process(delta: float) -> void:
	...
	for enemy in lockOnTargets:
		...
		if result and result.get("collider") == enemy: 
			los = true
			if !is_instance_valid(enemy) and enemy in lockOnChoice:
				lockOnChoice.erase(enemy)
			if "dying" in enemy and enemy in lockOnChoice:
				if enemy.dying == true:
					lockOnChoice.erase(enemy)
			elif enemy not in lockOnChoice && enemy != null: #&& enemy.dying != true: WARN
	...
	if lockOnChoice != null:
		for enemy in lockOnChoice:
			if enemy == null:
				lockOnChoice.erase(enemy)
			elif enemy == cam_root.target && cam_root.lockOn && "dying" in enemy and enemy.dying == false:
				if "barVisible" in enemy:
	...
	if inBetweenAtk:
		...
		if atkTimer >= inBetweenThreshold:
			#atkTimer = 0.0
			timerAtkStart = false
			inBetweenAtk = false
			isAttacking = false
	...
	if isSprintAttacking:
		if atkTimer <= 0.25:
			...
			if atkTimer >= 0.4/SWORD_SPEED && atkTimer < 0.7/SWORD_SPEED: #was 0.4-0.5
				swing4Move = true
			else:
				swing4Move = false
	elif swing3:
			if atkTimer >= 0.4/SWORD_SPEED && atkTimer < 0.6/SWORD_SPEED: #was 0.4-0.5
				swing3Move = true
			else:
				swing3Move = false
	elif swing2:
			if atkTimer >= 0.6/SWORD_SPEED && atkTimer < 0.8/SWORD_SPEED: #was 0.4-0.5
				swing2Move = true
			else:
				swing2Move = false
	elif swing1:
			if atkTimer >= 0.45/SWORD_SPEED && atkTimer < 0.65/SWORD_SPEED: #was 0.4-0.5
				swing1Move = true
			else:
				swing1Move = false
	...
	if Entityteam != team:
		if performParry:
			...
			if entityHurtType != 5:
				man.sparksActi()
				man.starActi()
				currPosture = currPosture - 10 #leave for stamina rework TODO
				hitParried = true
			...
			Events.hitstop(0.05, 0.3) # no dmg maybe + resource
		else:
			interrupt()
			man.abortOneShot()
			if stagger:
